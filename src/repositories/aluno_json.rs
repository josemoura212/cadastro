...
impl AlunoJsonRepo {
    ...
    pub fn read(&self) -> Option<Vec<Aluno>> {
        ...
        arquivo
            .read_to_string(&mut aluno_string)
        ...
        let alunos: Vec<Aluno> =
            serde_json::from_str(&aluno_string).expect("Erro ao desserializar o arquivo");

        Some(alunos)
    }

    pub fn write(&self, studants: Vec<Aluno>) {
        let alunos_string = serde_json::to_string(&studants).expect("Erro ao serializar o arquivo");

        let mut arquivo = File::create(&self.path).expect("Erro ao criar o arquivo");
        arquivo
            .write_all(alunos_string.as_bytes())
